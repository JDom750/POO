...
public class Lista_De_Tareas {
    ...
    public Tareas[] getLista_tareas() {
        return lista_tareas;
    }

    public List<Tareas> obtenerTareasRealizadasPorColaborador(String colaborador) {
        List<Tareas> tareasRealizadasPorColaborador = new ArrayList<>();
        for (int i = 0; i < cantidadTareas; i++) {
            Tareas tarea = lista_tareas[i];
            if (tarea.getEstado() == Estado.COMPLETA && tarea.getColaborador().equals(colaborador)) {
