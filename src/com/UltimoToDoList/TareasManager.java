...
public class TareasManager {
    ...
    public void marcarTareaComoRealizadaPorColaborador(Tareas tarea, String colaborador) {
        if (tarea != null) {
            tarea.marcarTareaComoRealizada(colaborador);
            System.out.println("Tarea '" + tarea.getDescripcion() + "' marcada como realizada por " + colaborador);
        }
    }

    public List<Tareas> obtenerTareasRealizadasPorColaborador(String colaborador) {
        return listaTareas.obtenerTareasRealizadasPorColaborador(colaborador);
    }
}
